 ...
 * Implements a graceful fallback mechanism to an offline state if configuration is missing.
 * This guarantees the application remains entirely playable even without a configured backend.
 * 
...
/**
 * Dictionary mapping Formula 1 constructors to their respective brand colors.
 * Utilized by the UI to dynamically alter Tailwind CSS utility classes and SVG assets,
 * allowing the user to deeply personalize their application theme.
 */
const CONSTRUCTORS = {
...
/**
 * The core Achievement Definition Dictionary containing 30 unique unlockable milestones.
 * Note: Emojis are strictly typed as strings. This architectural choice prevents React 
 * from throwing rendering errors by attempting to parse an object as a React node.
 */
...
/**
 * Standardizes time formatting for the main timer UI.
 * Pads the raw mathematical decimal with zeroes to simulate a high-precision physical stopwatch.
 * 
 * @param {number|null} time - Raw time in seconds.
 * @returns {string} Formatted string strictly padded to 3 decimal places.
 */
...
/**
 * Derives the Population Standard Deviation for an array of race times.
 * This serves as the core metric for tracking a user's biological "consistency".
 * 
 * @param {number[]} arr - The array of historical reaction times.
 * @returns {number} The calculated variance/standard deviation constraint.
 */
 ...
 * @param {number} stdDev - The user's calculated consistency.
 * @returns {string} A targeted tip string.
 */
 ...
 * Modern browsers block audio playback until the user physically interacts with the DOM.
 * This function is primed inside click handlers to ensure audio plays reliably on mobile.
 */
let audioCtx = null;
const initAudio = () => {
 ...
 * Creates synthetic sine/square wave "beeps" synchronously with the visual light updates,
 * eliminating the need to download or buffer external .mp3 files.
 * 
 * @param {string} type - Differentiates the low pitched red light from the high pitched 'Go' signal.
 */
 ...
 * Ensures properties revert to clean bounds when restarting sequences.
 */
 ...
 * By utilizing the `useReducer` pattern instead of multiple independent `useState` hooks, 
 * we guarantee atomic state transitions. This prevents the React queue from encountering 
 * race conditions during ultra high-frequency user interactions (like sub-100ms clicks).
 * 
 * @param {Object} state - The current game state.
 * @param {Object} action - Action dispatcher payload.
 * @returns {Object} The derived next state.
 */
function gameReducer(state, action) {
  switch (action.type) {
    ...
    case 'FINISH':
      // Practice mode evaluations actively prevent incrementing the global competitive streak counter.
 ...
 * @param {Object} props - Tailwind CSS classNames for styling.
 */
...
/** 
 * Vector graphic of a Formula 1 car, accepting dynamic color injection. 
 * @param {Object} props - Contains classNames and the hex color payload.
 */
 ...
 * Wrapped in `React.memo` to proactively block expensive DOM re-renders 
 * when the main game sequence updates at 1-millisecond intervals.
 */
const ThemeToggleMenu = React.memo(({ themePref, isDark, showThemeMenu, setShowThemeMenu, handleThemeSelect }) => (
  <div className="relative">
    ...
    {showThemeMenu && (
      <>
        {/* Invisible overlay used to capture outside clicks, safely closing the dropdown */}
...
/**
 * Mathematical particle physics generator.
 * Triggers a localized CSS keyframe animation burst when the user sets a new personal record.
 * Uses a cleanup effect to unmount itself automatically after 2 seconds, freeing DOM nodes.
 */
...
/**
 * The physical representation of the F1 starting gantry.
 * Leverages deeply customized Tailwind box-shadow properties to maintain an illusion of depth
 * and responds fluently to variable viewport widths (mobile vs desktop).
 */
 ...
 * Intercepts the user's master history arrays to compute standard metrics dynamically.
 * Features a histogram that scales purely via CSS width properties, bypassing heavy charting dependencies.
 */
const DashboardModal = React.memo(({ isOpen, onClose, isDark, fullHistory, falseStarts }) => {
  if (!isOpen) return null;
  
  // Isolate valid reaction blocks to ensure pure mathematical averages
  ...
  // Memory slice sorting for Median and Fast-5 tracking
  ...
  const recent5 = [...validHistory].reverse().slice(0, 5); 
  ...
  // Distribution buckets for rendering the histogram
  ...
  // Identify peak density relative sizing of the CSS progress bars
...
/**
 * Global Achievements Presentation Layer.
 * Renders the milestone array in a responsive grid. Maps unlocked achievements
 * to fully opaque components, defaulting locked achievements to a disabled grayscale layout.
 */
 ...
 * Enables users to mutate their display name and saves changes instantly back to the database.
 */
const LeaderboardModal = React.memo(({ isOpen, onClose, user, currentUsername, onUpdateUsername, isDark, allScores }) => {
  ...
  // Derived array filters and slices in Javascript memory to follow strict non-complex querying rules.
  const filteredScores = useMemo(() => {
    let scores = [...allScores].filter(s => s.bestTime > 0);
    if (filter === 'today') {
      const today = new Date().setHours(0,0,0,0);
      scores = scores.filter(s => s.updatedAt != null && s.updatedAt >= today);
    }
    // Perform memory sort to enforce strict requirements
    scores.sort((a, b) => a.bestTime - b.bestTime);
    return scores.slice(0, 10);
  }, [allScores, filter]);

  // Compute absolute rank based on globally sorted array mapping
  const userRank = useMemo(() => {
    const sorted = [...allScores].filter(s => s.bestTime > 0).sort((a, b) => a.bestTime - b.bestTime);
    return sorted.findIndex(s => s.id === user?.uid) + 1;
  }, [allScores, user]);
...
/** Contextual documentation interface exposing attribution and rule definitions. */
 ...
 * Orchestrates local state cache, real-time Firestore synchronization, and global 
 * gameplay flow through heavily optimized hooks.
 * 
...
export default function App() {
  ...
  /**
   * APPLICATION EFFECT: OS Theme System Linkage
   * Dynamically tracks the OS preference via `matchMedia` and updates the app layout accordingly.
   */
  useEffect(() => {
  ...
  /**
   * APPLICATION EFFECT: Anonymous Backend Authentication
   * Facilitates a localized pseudo-session tying leaderboard records to this specific browser context.
   */
  useEffect(() => {
    if (!auth) { 
      setUser({ uid: 'local_user_' + Math.random().toString(36).substring(2, 8) }); 
      setUsername('LocalRacer'); 
      setAuthReady(true); 
      return; 
    }
    ...
    const initAuth = async () => { 
      try { 
        ...
        if (currentUser) {
           setUser(currentUser);
           setUsername(`Racer_${currentUser.uid.substring(0, 5)}`);
        }
      } catch (err) { 
  ...
  /**
   * APPLICATION EFFECT: Firestore Architecture Snapshot Bindings
   * Pulls the absolute active leaderboards asynchronously while conforming to constrained bandwidth boundaries.
   */
  useEffect(() => {
    if (!user || !authReady) return;
    
    let timeout;
    const fallbackScores = [ { id: 'm1', username: 'Max V.', bestTime: 0.185, attempts: 42, updatedAt: Date.now() }, { id: 'm2', username: 'Lewis H.', bestTime: 0.192, attempts: 38, updatedAt: Date.now() } ];
  ...
  const globalStats = useMemo(() => {
    let totalTime = 0, count = 0; allScores.forEach(s => { if (s.bestTime != null && s.bestTime > 0) { totalTime += s.bestTime; count++; } });
   ...
   * Master ruleset parsing logic. Utilizing a state functional updater pattern `prevAchs => ...` 
   * breaks the dependency lifecycle trap, keeping our high speed racing loop from crashing React.
   * 
   * @param {Object} metrics - Packaged performance data tracking the user's just-completed attempt.
   */
  const evaluateAchievements = useCallback((metrics) => {
    ...
    setAchievements(prevAchs => {
      const newAchs = new Set(prevAchs);
  ...
  /**
   * Bootstraps the racing sequence.
   * Calculates random physical delays and schedules independent asynchronous timeout blocks.
   */
  ...
  /**
   * The Core Game Loop Interaction Controller.
   * Compares the localized timestamp offsets against the reducer states to determine validity.
   * Integrates haptics heavily for a responsive mobile experience.
   * 
   * @param {Event} e - The raw DOM interaction payload.
   */
  ...
  /**
   * Input Deduplication Handler.
   * Solves a highly critical mobile Safari/Chrome quirk where a singular physical screen tap 
   * dispatches both a raw 'pointerdown' AND a synthesized 'click' 300ms later.
   * Prevents the secondary ghost click from triggering immediate false starts.
   */
  ...
  /**
   * System Share Hook.
   * Maps to the native mobile system UI sharing sheet if the OS supports it.
   */
   ...
   * Integrates a fluid viewport mapping (min-h-[100dvh]) to fix overflow cuts natively seen on 
   * iOS/Android webkit instances. Replaced the static h-screen which prevented vertical scrolling.
   */
  if (showLanding) {
    return (
      <div className={`flex flex-col min-h-[100dvh] font-sans overflow-y-auto overflow-x-hidden transition-colors duration-300 relative ${isDark ? 'bg-[#0f0f13] text-gray-100' : 'bg-gray-50 text-gray-900'} theme-container`}>
         <div className="absolute inset-0 pattern-grid opacity-5 pointer-events-none min-h-full"></div>
         ...
         <div className="flex-1 flex flex-col items-center justify-center p-6 py-12 z-10 relative min-h-full">
            <F1CarIcon color={CONSTRUCTORS[selectedCar].color} className="w-32 h-auto mb-8 drop-shadow-2xl animate-in slide-in-from-left-8 duration-500 shrink-0" />
            <h1 className="text-4xl sm:text-6xl font-black tracking-tighter mb-2 text-center leading-none shrink-0">Reaction<span style={{color: CONSTRUCTORS[selectedCar].color}}>Test</span></h1>
            <p className={`text-lg sm:text-xl font-medium mb-12 text-center max-w-md shrink-0 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>The premiere motorsport starting light simulator. Test your reflexes against the world.</p>
            
            <div className={`p-6 rounded-3xl w-full max-w-md mb-8 shadow-xl border shrink-0 ${isDark ? 'bg-gray-800/50 border-gray-700' : 'bg-white border-gray-200'}`}>
            ...
            <button onClick={() => setShowLanding(false)} className={`w-full max-w-md shrink-0 ${CONSTRUCTORS[selectedCar].textClass} font-black text-xl py-5 rounded-full shadow-2xl transition-transform active:scale-95 ${CONSTRUCTORS[selectedCar].class} ${CONSTRUCTORS[selectedCar].hover}`}>
              PULL TO GRID
            </button>
         </div>
      </div>
  ...
  return (
    ...
    >
      ...
      {/* Embedded Copy Notification Toast Overlay */}
      {toastMessage && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-[100] bg-green-500 text-white px-5 py-2.5 rounded-full font-bold text-sm shadow-2xl animate-in slide-in-from-top-4 flex gap-2 items-center">
